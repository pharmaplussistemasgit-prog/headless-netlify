                ...
                <div className="bg-white shadow-sm p-6 md:p-8">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div className="relative">
                            ...
                            <div className="absolute bottom-[-1px] left-0 w-full h-[3px] bg-purple-600 rounded-full"></div>
                        </div>

                        <Link href="/categoria/belleza" className="text-gray-400 text-sm font-medium hover:text-purple-600 flex items-center gap-1 transition-colors">
                    ...
                    <div className="relative group">
                        ...
                        <button
                            ref={prevRef}
                            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-20 w-8 h-8 flex items-center justify-center text-gray-400 hover:text-purple-600 transition-colors bg-white shadow-sm rounded-full disabled:opacity-30 border border-gray-100"
                        >
                        ...
                        <button
                            ref={nextRef}
                            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-20 w-8 h-8 flex items-center justify-center text-gray-400 hover:text-purple-600 transition-colors bg-white shadow-sm rounded-full disabled:opacity-30 border border-gray-100"
                        >
                        ...
                        >
                            {products.map((product) => {
                                ...
                                // Simulate discount if none for visuals matching requirements
                                    ...
                                    <SwiperSlide key={product.id} className="h-full">
                                        <div className="group/card relative flex flex-col bg-white h-full">
                                            ...
                                            <div className="bg-yellow-100 text-[10px] text-yellow-800 px-2 py-1 rounded w-full mb-2 truncate">
                                                Primera compra App y Web
                                            </div>

                                            {/* Title */}
                                            <Link href={`/producto/${p.slug}`} className="mb-2 block">
                                                <h4 className="text-sm font-medium text-gray-700 line-clamp-2 min-h-[40px] group-hover/card:text-purple-600 transition-colors">
                                                    {p.name}
                                                </h4>
                                            </Link>
